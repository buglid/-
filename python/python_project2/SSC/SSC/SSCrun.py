...
def SSC_test(data, k,lmbda):
    ...
    OptM = 'L1ED'  # OptM can be {'L1Perfect','L1Noise','Lasso','L1ED'}
    ...
    data = data.transpose()
    data = DataProjection(data, r, 'NormalProj')
    m = int(data.shape[1] / k)
    n = data.shape[1] % k
    if k == 2:
        print(k)
        s = np.concatenate((1 * np.ones([1, m]), 2 * np.ones([1, m + n])), axis=1)
    elif k == 3:
        print(k)
        s = np.concatenate((1 * np.ones([1, m]), 2 * np.ones([1, m]), 3 * np.ones([1, m + n])), axis=1)
    elif k == 4:
        s = np.concatenate(
            (1 * np.ones([1, m]), 2 * np.ones([1, m]), 3 * np.ones([1, m]), 4 * np.ones([1, m + n])),
            axis=1)
    else:
        s = np.concatenate(
            (1 * np.ones([1, m]), 2 * np.ones([1, m]), 3 * np.ones([1, m]), 4 * np.ones([1, m]),
             5 * np.ones([1, m + n])),
            axis=1)

    # Xp = DataProjection(data, r, 'NormalProj')
    CMat = SparseCoefRecovery(data, Cst, OptM, lmbda)  # shape(100, 100)
    # Make small values 0
    ...
    if Fail == False:
        CKSym = BuildAdjacency(CMatC, 2)
        Grps = SpectralClustering(CKSym, k)

        labels = []
        for i in range(k):
            labels.append(sum(Grps == i))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--lmbda', default='0.001', help=u'lmbda')
    ...
    labels, data_res = SSC_test(X_minMax, k,lmbda)
